...
# User parameters
teams = range(26,28+1) # Teams list
...
consoles = ['SONAR1','PERISCOPE','TMA1','TMA2','TPC']
...
### CHANGE for this version, keep in the range outliers (will hit the PE threshold)
        df_sl = pd.read_csv('sl_sonaroutliers_%s.csv' % condlabel) # All solutions entered. Includes initial solution set by sonar
...
### CHANGE: Set default PE, update later if there's a solution
                    ...
                    df_tssl.loc[tssl,'tssl_ts_bearing'] = df_ts[tstpcrit]['ts_bearing'].iloc[0]   # Get x/y data etc            
...
# 4. Get solution data                
                    # Solution lodged yet?    
                    if (df_sl[tscrit].shape[0] > 0): # If any solution
                        if (tp_time >= df_sl[tscrit]['sl_post_time'].iloc[0]): # If the first solution submitted
                            # Solution x/y    
                            if (sl_c < df_sl[tscrit].shape[0]): # if there are new solutions to come
                                if (df_sl[tscrit]['sl_post_time'].iloc[sl_c] == tp_time): # if new solution has just been submitted
                                    sl_id = df_sl[tscrit]['sl_sid'].iloc[sl_c]
                                    # Update x/y from solution start time
                                    sl_startx = df_sl[tscrit]['sl_x'].iloc[sl_c].copy()
                                    sl_starty = df_sl[tscrit]['sl_y'].iloc[sl_c].copy()
                                    sl_bearing = df_sl[tscrit]['sl_bearing'].iloc[sl_c].copy()
                                    sl_course = df_sl[tscrit]['sl_course'].iloc[sl_c].copy()
                                    sl_speed = df_sl[tscrit]['sl_speed'].iloc[sl_c].copy()
                                    sl_timediff = tp_time - df_sl[tscrit]['sl_time'].iloc[sl_c].copy()
                                    ...
                                    sl_bearing = get_bearing(sl_newx,df_tssl.loc[tssl,'tssl_ownship_x'],sl_newy,df_tssl.loc[tssl,'tssl_ownship_y'])
                            else:
                                # Update from last timepoint
                                sl_newx = sl_newx + np.sin(np.deg2rad(sl_course)) * (ypm * (tpdur/60) * sl_speed);
                                sl_newy = sl_newy + np.cos(np.deg2rad(sl_course)) * (ypm * (tpdur/60) * sl_speed);
                            ...
                            df_tssl.loc[tssl,'tssl_sl_bearing'] = sl_bearing
                            df_tssl.loc[tssl,'tssl_PE'] = min(ospa_c, positionError(df_tssl.loc[tssl,'tssl_ts_x'], sl_newx, df_tssl.loc[tssl,'tssl_ts_y'], sl_newy))
                            df_tssl.loc[tssl,'tssl_bearingError'] = bearingError(df_tssl.loc[tssl,'tssl_ts_bearing'], df_tssl.loc[tssl,'tssl_sl_bearing'])
...
### CHANGE: Calculate this for any detected contact, even if no solutions on them
            if df_tssltp.loc[tp,'tp_detectedCount'] > 0: # Only calculate this for rows with a solution
                ...
                df_tssltp.loc[tp,'tp_meanBearingError'] = df_tssl[tsslcrit]['tssl_bearingError'].mean()             
        ...
        # Get most recently completed atwit
            ...
            if atw_start_time >= 300:
                ...
                df_tssltp.loc[tp,'tp_atw_all'] = (df_tssltp.loc[tp,'tp_atw_SONAR1'] + df_tssltp.loc[tp,'tp_atw_PERISCOPE'] + df_tssltp.loc[tp,'tp_atw_TMA1'] + df_tssltp.loc[tp,'tp_atw_TMA2'] + df_tssltp.loc[tp,'tp_atw_TPC'])/6
