...
maindir = ('C:\\Users\\User\\PycharmProjects\\CRUSE')
...
# 2. Calculate x/y for solution
        ...
        for sl, row in df_sl.iterrows(): 
            navcrit = (df_nav.loc[:,'nd_time']==df_sl.loc[sl,'sl_time'])
...
# 4. For each SL contact, find the closest TS current visitble
        for sl_id in sl_all_cons: 
            ...
            if ts_bears['bear_diffs'].min() > 1:
                flagfile.write('*** FLAG *** SCID assigned to a contact with bearing error greater than 1\n')
            if sum(ts_bears['bear_diffs'].nsmallest(2)<1) > 1: #i.e. if more than 2 TS are within 1 degree
                flagfile.write('*** FLAG *** Two possible contacts within 1 degree of SCID\n')
        ...
        for ts_id in ts_all_cons:
            ...
            if df_sl[tscrit].shape[0] > 0:
                if pd.unique(df_sl[tscrit]['sl_sid']).shape[0] > 1: #If multiple SLs assigned to same SD
                    flagfile.write('*** FLAG *** More than one SL assigned to TS %d' % ts_id) 
...
# 12. Save sonar data
        ...
        for ts_id in ts_all_cons:
            ...
            stcrit = (df_st['st_entity_id']== ts_id)
...
# 13. Save original data then remove remove outliers                       
        ...
        # Save all data include sonar solutions and outliers to processed data folder
        df_son.to_csv('son_%s.csv' % condlabel)
        df_sl.to_csv('sl_all_%s.csv' % condlabel)
        
        df_sl['sl_team']  = team
        df_sl['sl_session'] = session 
        df_sl['sl_config'] = configcond
        df_sl['sl_integ'] = intcond        
        
        # Combine sll_all data across teams
        if firstcompleted == 0:
...
# 14. Remove outliers.        
        ...
        ### NOTE RT calculations are performed before bearing error and RT outlier trials are removed. So RT is still calculculated from the most recent previous solution, even if that solution was an outlier  
        # Bearig error outliers
        bearcrit = (df_sl['sl_bearingError'] <= bearerrmax)
        if df_sl['sl_range'].max() > bearerrmax:
...
# Save final sl data solution data
        df_sl = df_sl.reset_index(drop=True)
        df_sl.to_csv('sl_%s.csv' % condlabel) 
        
        # Combine data across teams
        if firstcompleted == 0:
